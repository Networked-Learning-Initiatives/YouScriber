            ...
            .then((perms) => {
              ...
              if (perms && ((perms.hasOwnProperty('canAdmin') && perms.canAdmin) || (perms.hasOwnProperty('canComment') && perms.canComment && comment.authorId === user.id))) {
                ...
                comment.destroy()
                  ...
                  .catch(function (err) {
                    res.status(500).send('some problem dropping comment' + err)
                  })
              }
            })
        })
...
app.post('/api/comments/:cid/time', function (req, res) {
  updateCommentTime(req.params.cid, req.body.time)
    ...
    .catch(function (err) {
      res.status(500).send('some problem updating comment time' + err)
    })
})
...
app.post('/api/comments/:cid/content', function (req, res) {
  updateCommentContent(req.params.cid, req.body.content)
    ...
    .catch(function (err) {
      res.status(500).send('some problem updating comment content' + err)
    })
})
